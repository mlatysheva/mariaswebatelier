...
describe('classNames', () => {
  ...
  test('Should return class names for mods', () => {
    ...
    expect(classNames('some-class', { hovered: true, scrollable: true }, ['class1', 'class2'])).toBe(expected);
  });

  test('Should return class names for mods with false', () => {
    const expected = 'some-class class1 class2 hovered';
    expect(classNames('some-class', { hovered: true, scrollable: false }, ['class1', 'class2'])).toBe(expected);
  });
})